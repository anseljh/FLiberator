...
### Phase 3 — Document boundaries & the citation index
- We already found a table-of-contents-like block (`<div class="IndexItem">` entries with `#ID=FS20250001.01 --#`-style anchors) near the front of `fs2025.nxt`. Extract all such anchors across the file to build a citation → approximate-byte-offset map.
- Determine whether there's a *true* random-access index (binary offset table) elsewhere in the file, or whether the practical answer is "collect anchors via one linear pass, then treat each anchor as the start of a document run until the next anchor." Given the file sizes involved (240MB for the full statutes), a single linear pass is entirely tractable even if no faster index is found — don't over-invest in finding a binary index if the linear-scan approach works.
