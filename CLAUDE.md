...
2. **Unzip** the archive, also into `download/` — `download/FLLawDL2025/Library/*.nxt` ends up laid out identically to the frozen reference copy (see "Working conventions" below). Steps 1-2 run as part of `uv run fliberate --download`, which then decodes the edition it just fetched rather than the hardcoded default path.
