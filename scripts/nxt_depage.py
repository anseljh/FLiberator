...
def parse_pages(data: bytes) -> tuple[dict, dict]:
    ...
    for page in range(1, len(data) // PAGE_SIZE):
        ...
        starts = [header_len] + sorted(s & 0x1FFF for s in slots[:-1])
        fragments[page] = [
...
def load_records(path: pathlib.Path | None = None) -> list[bytes]:
    """Convenience entry point for the other analysis scripts."""
    path = path or pathlib.Path("FLLawDL2025/Library/fs2025.nxt")
    return reconstruct(path.read_bytes())


def main() -> None:
    path = pathlib.Path(sys.argv[1] if len(sys.argv) > 1 else "FLLawDL2025/Library/fs2025.nxt")
    data = path.read_bytes()
    records = reconstruct(data)

    title_re = re.compile(rb"<title>([^\x13]{0,80})\x13\x37\x08</title>", re.IGNORECASE)
    titled = sum(1 for r in records if title_re.search(r))
    closed = sum(1 for r in records if r.rstrip(b"\x00").endswith(b"</html>"))
    total = sum(len(r) for r in records)

    print(f"{path}: {len(data):,} bytes = {len(data) // PAGE_SIZE:,} pages")
    print(f"documents reassembled: {len(records):,} ({total:,} bytes of content)")
    print(f"  with an intact <title>...</title>: {titled:,} ({titled / len(records):.2%})")
    print(f"  ending in </html>:                 {closed:,} ({closed / len(records):.2%})")
    print(f"  same title scan, read linearly:    {len(title_re.findall(data)):,}")
